...
export default async function DoctorTeleconsultPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Teleconsult"
        subtitle="Send Google Meet links to patients on WhatsApp for video consultations"
      />
      ...
      {upcoming.length === 0 && past.length === 0 ? (
        <EmptyState
          ...
          description="Sessions are created when patients book video consultations."
        />
      ) : (
                    ...
                    <Card key={session.id} hover>
                      ...
                      <div className="mb-4">
                        {hasMeetLink ? (
                          ...
                          <Badge variant="warning">Send Google Meet link</Badge>
                        )}
                      </div>
                      ...
                      <Link href={`/doctor/teleconsult/${session.id}`}>
                        <Button className="w-full gap-2">
                          ...
                          {hasMeetLink ? "View session" : "Send Meet link"}
                        </Button>
                      </Link>
                    </Card>
                  );
