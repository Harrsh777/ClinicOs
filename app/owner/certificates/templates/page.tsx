...
export default async function OwnerCertificateTemplatesPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        action={
          <Link href="/owner/certificates" className="clinic-btn clinic-btn-secondary text-xs">
          ...
          {systemTemplates.map((tmpl) => (
            <div key={tmpl.id} className="flex flex-col justify-between rounded-2xl border border-[var(--border)] bg-white p-5 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="mt-4 pt-3 border-t border-[var(--border)]">
                <Link href={`/owner/certificates/new?templateId=${tmpl.id}`} className="clinic-btn clinic-btn-primary w-full text-center text-xs">
                  Use This Template
                </Link>
              </div>
            </div>
            ...
            {customTemplates.map((tmpl) => (
              <div key={tmpl.id} className="flex flex-col justify-between rounded-2xl border border-[var(--border)] bg-white p-5 shadow-sm">
                ...
                <div className="mt-4 pt-3 border-t border-[var(--border)] flex gap-2">
                  <Link href={`/owner/certificates/new?templateId=${tmpl.id}`} className="clinic-btn clinic-btn-primary flex-1 text-center text-xs">
                    Use Template
                  </Link>
                </div>
              </div>
