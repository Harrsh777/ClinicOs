...
export function QRCodeDisplay({ clinicSlug, clinicName }: QRCodeDisplayProps) {
  ...
  const portalUrl = `${origin}/c/${clinicSlug}`;
  const checkInUrl = `${origin}/c/${clinicSlug}/check-in`;

  return (
    <div className="space-y-6">
      <div className="text-center">
        <p className="text-sm font-medium mb-1">Patient Portal</p>
        <p className="text-xs text-[var(--text-muted)] mb-3">
          Share this link — patients can book, pay, and get tokens online
        </p>
        <div className="inline-block p-4 bg-white rounded-[var(--radius-lg)] border border-[var(--border)]">
          ...
          <img
            ...
            alt={`QR code for ${clinicName} patient portal`}
            width={200}
            height={200}
            className="mx-auto"
          />
        </div>
        <p className="mt-3 text-xs font-mono break-all text-[var(--brand-600)]">/c/{clinicSlug}</p>
        <div className="mt-2 flex flex-wrap justify-center gap-2">
          <Link href={`/c/${clinicSlug}`} target="_blank" className="clinic-btn clinic-btn-secondary clinic-btn-sm inline-flex">
            Open Portal
          </Link>
          <Link href={`/c/${clinicSlug}/walk-in`} target="_blank" className="clinic-btn clinic-btn-primary clinic-btn-sm inline-flex">
            Walk-in Queue
          </Link>
        </div>
      </div>
